package com.rjzd.baby.ui.widget.edit;
 ...
 * descriptions: 统计剩余字数的自定义 Edittext
 */
public class CountEditText extends RelativeLayout {
    ...
    public static final int REMAINING = 1;//类型1(单数类型)
    public static final int PERCENTAGE = 2;//类型2(百分比类型)
    private EditText etContent;//文本框
    private TextView tvNum;//字数显示TextView
    private int mMode ;//类型
    private int maxLength = 50;//最大字符
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if(array.hasValue(R.styleable.CountEditText_ce_countMode)){
            mMode = array.getInt(R.styleable.CountEditText_ce_countMode,PERCENTAGE);
        }
        String mHintText = array.getString(R.styleable.CountEditText_ce_hint);
        ...
        if(mMode == REMAINING){//类型1
            tvNum.setText(String.valueOf(maxLength));
        }else if(mMode == PERCENTAGE){//类型2
